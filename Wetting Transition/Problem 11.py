...
for i in x:
    fluidDensities = run((i/beta) - 2.5 * epsilon, 1 / beta, epsilon, epsilon_w1, Lx, Ly, potential = True)
    bulkGasDensities = run((i/beta) - 2.5 * epsilon, 1 / beta, epsilon, epsilon_w1, Lx, Ly, potential = False)
    y1.append(np.sum(fluidDensities - bulkGasDensities))
    
    fluidDensities = run((i/beta) - 2.5 * epsilon, 1 / beta, epsilon, epsilon_w2, Lx, Ly, potential = True)
    bulkGasDensities = run((i/beta) - 2.5 * epsilon, 1 / beta, epsilon, epsilon_w2, Lx, Ly, potential = False)
    y2.append(np.sum(fluidDensities - bulkGasDensities))

py.style.use("dark_background")
py.plot(x, y1, color = 'c', label = "$βƐ_w$ = 0.5")
py.plot(x, y2, color = 'm', label = "$βƐ_w$ = 2")
py.xlabel("$β$($μ$ - $μ_{coex}$)", size = 20)
py.ylabel('$Γ$', size = 20)
py.legend(prop={'size': 15})
py.tight_layout()
py.savefig('Problem11.pdf')
py.show()
